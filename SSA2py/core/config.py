...
import yaml, logging, functools, sys, os, time as tm
...
def init(evnt, _cfg):
    ...
    global cfg, org, mag, eventsdir, eventdir, inventorydir,\
           ...
           timerules, logger, blacklist, inv, stations_, stations_status, grid, tables,\
           comp, fi, q, model, st_Synth, mt, data_sit, phase, scanningRules, gridRules, stations,\
           savedir, whitelist, origin_p, posx, posy, raw, job, Qual 
    ...
    blacklist = readLists(cfg['Download Rules']['Blacklist']) #Blacklisted stations
    whitelist = readLists(cfg['Download Rules']['Whitelist']) #Whitelisted stations
