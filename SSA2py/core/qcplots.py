...
import obspy, math, os, gc, matplotlib
...
def SNRPlot(path, streams, traces, res):
    ...
    if len(streams)<=20:
        ...
        num_p = math.ceil(len(streams)/20)

    for p in range(num_p):
        #Get the traces
        try:
           streams_ = streams[p*20:(p*20)+20]
           res_ = res[p*20:(p*20)+20]
        except:
           streams_ = streams[p*20:]
           res_ = res[p*20:]
        #Columns per plot
        if len(streams_)<=5:
            col_p = 1
        else:
            col_p = math.ceil(len(streams_)/5)
        ...
        fig = plt.figure(1, figsize=(3.3*col_p, 11))
        for k in range(Tot):
        ...
        for k in range(len(axes)):
            ...
            ymin = np.min(tr.data) - 0.5
            ymax = np.max(tr.data) + 0.5
...
def CLIPPlot(path, streams, traces, res):
    ...
    for p in range(num_p):
        ...
        fig = plt.figure(1, figsize=(3.4*col_p, 11))
        for k in range(Tot):
            # add every single subplot to the figure with a for loop
            ax = fig.add_subplot(Rows,Cols,Position[k])
        axes = fig.axes
        ...
        for k in range(len(axes)):
            ...
            if len(res_[k]['CLIP'])==1:
                ...
                if res_[k]['CLIP'][0]==False:
                    ...
                    axes[k].text(0.65, 0.1, 'Removed', weight='bold', transform=axes[k].transAxes,\
