...
def trigSTALTA(trace, STA, LTA):
    ...
    #Filter the signal (Bandpass 1-25 Hz)
    trace.filter('bandpass', freqmin=1.0, freqmax=25.0)
    ...
    # Scale between 0-1
    scaler = MinMaxScaler()
    normalized_cft = scaler.fit_transform(cft.reshape(-1, 1))

    start_spot = np.argmax(normalized_cft)

    return (start_spot, cft)
...
def trigKurtosis(trace, C):
    ...
    trace.filter('bandpass', freqmin=1, freqmax=25)
