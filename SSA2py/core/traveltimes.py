...
import subprocess, sys, shutil, multiprocessing,\
       os, obspy, yaml, skfmm, math
import numpy as np
...
def interpGrid(model, gx, gy, gz):
    ...
    rbf = RBFInterpolator(points, model[:,3], kernel='linear', smoothing=0)

    points_n = np.column_stack((gx.flatten(), gy.flatten(), gz.flatten()))
    V = rbf(points_n)
