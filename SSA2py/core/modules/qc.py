...
def checkSNR(trace, Qdec, triggerType, SNR_Window, SNR_thres):
    ...
    if triggerType == 'STALTA':
        trigs = trigSTALTA(trace.copy(), STA = 1, LTA = 20, threshold=15)
    else:
        Kurtosis_window = 5  
        trigs = trigKurtosis(trace, 1-(trace.stats.delta/Kurtosis_window), threshold=30)    
    #No triggers
    if len(trigs[0])==0:
        Qdec['SNR'] = [False]
    else:
        try:
            SNR = SNRampl(trace.copy(), trigs[0][0], a_time=SNR_Window)
            if SNR >= SNR_thres: #SNR threshold
                  Qdec['SNR'] = [True, trigs[1], trigs[0][0], SNR] #[True or False, CF, Arrival, SNR value]
            else:
                  Qdec['SNR'] = [False, trigs[1], trigs[0][0], SNR]
        except:
            Qdec['SNR'] = [False]
    return Qdec
...
def checkTIME(trace, Qdec, triggerType, P_win):
    ...
    try:
        ...
        if len(trigs)==0:
            ...
            arr = arrival(config.org.latitude,\
            ...
            obs_arr = trace.stats.starttime +\
                      (trigs[0][0]/trace.stats.sampling_rate) 
