...
def atlas(inv, stations_used, stations_notused, evla, evlo, evdepth,\
    ...
    #Add Towns
    #Read the shapefile
    dist_d = kilometer2degrees(maxdist)
    ...
    for sta in stations_used:
        ax1.plot(inv.select(station=sta.split('.')[1])[0][0].longitude,\
                 ...
                 "^", color='springgreen', markersize=10, markeredgecolor='k', markeredgewidth=0.5, transform=ccrs.Geodetic())

    #Plot unsed stations
    for sta in stations_notused:
        ax1.plot(inv.select(station=sta.split('.')[1])[0][0].longitude,\
                 inv.select(station=sta.split('.')[1])[0][0].latitude,\
                 "^", color='silver', markersize=10, markeredgecolor='k', markeredgewidth=0.5, transform=ccrs.Geodetic())
    #Plot meridians and parallels
    if meridians==True:
    ...
    #Inset map
    ax_inset = fig.add_axes([0.712, 0.684, 0.185, 0.205], projection=ccrs.PlateCarree())
