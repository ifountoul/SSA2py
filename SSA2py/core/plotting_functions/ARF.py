...
def ARFplots(data, inv, evla, evlo, evdepth, st, origin, tt,\
    ...
    lat = lat[np.nonzero(lat)]; lon = lon[np.nonzero(lon)];
    ...
    ax3.scatter(0,-0.5,400, marker='*', color='red', edgecolor='k', zorder=5, label = 'Hypocenter')
    ...
    ax3.bar(theta, radii, width=width, bottom=bottom, color='b', edgecolor='k',alpha=0.2, label='Histogram')
    ...
    ax3.plot(x, radii, linewidth=2, color='firebrick', zorder=3, label='VM Distr.' + '\n' + 'Mu=' + str(round(np.degrees(mu),1)) + '$^\circ$' + 'Kappa=' + str(round(kappa,1)))
